...
class EmojiManager:
    ...
    def __init__(self):
        ...
        self.vlm = LLM_request(
            model=global_config.vlm, temperature=0.3, max_tokens=1000
        )
        self.llm_emotion_judge = LLM_request(
            model=global_config.llm_normal_minor, max_tokens=60, temperature=0.8
            ...
            try:
                ...
                all_emojis = list(
                    self.db.db.emoji.find(
                        {}, {"_id": 1, "path": 1, "embedding": 1, "discription": 1}
                    )
                )
                ...
                emoji_similarities = [
                    (
                        emoji,
                        cosine_similarity(text_embedding, emoji.get("embedding", [])),
                    )
                ...
                if selected_emoji and "path" in selected_emoji:
                    ...
                    self.db.db.emoji.update_one(
                        {"_id": selected_emoji["_id"]}, {"$inc": {"usage_count": 1}}
                    )
                    logger.success(
                        f"找到匹配的表情包: {selected_emoji.get('discription', '无描述')} (相似度: {similarity:.4f})"
                    )
                    # 稍微改一下文本描述，不然容易产生幻觉，描述已经包含 表情包 了
                    return selected_emoji["path"], "[ %s ]" % selected_emoji.get(
                        "discription", "无描述"
                    )
    ...
    async def _get_emoji_discription(self, image_base64: str) -> str:
        ...
        try:
            prompt = (
                "这是一个表情包，使用中文简洁的描述一下表情包的内容和表情包所表达的情感"
            )
    ...
    def check_emoji_file_integrity(self):
        ...
        try:
            ...
            for emoji in all_emojis:
                try:
                    if "path" not in emoji:
                        logger.warning(
                            f"发现无效记录（缺少path字段），ID: {emoji.get('_id', 'unknown')}"
                        )
                    ...
                    if "embedding" not in emoji:
                        logger.warning(
                            f"发现过时记录（缺少embedding字段），ID: {emoji.get('_id', 'unknown')}"
                        )
                        self.db.db.emoji.delete_one({"_id": emoji["_id"]})
                        removed_count += 1
                        continue
            ...
            if removed_count > 0:
                ...
                logger.info(
                    f"清理前总数: {total_count} | 清理后总数: {remaining_count}"
                )
            else:
