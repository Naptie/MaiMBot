    ...
    def load_config(cls, config_path: str = None) -> "BotConfig":
        ...
        def personality(parent: dict):
            ...
            if len(personality) >= 2:
                ...
                config.PROMPT_PERSONALITY = personality_config.get(
                    "prompt_personality", config.PROMPT_PERSONALITY
                )
            logger.info(
                f"载入自定义日程prompt:{personality_config.get('prompt_schedule',config.PROMPT_SCHEDULE_GEN)}"
            )
            config.PROMPT_SCHEDULE_GEN = personality_config.get(
                "prompt_schedule", config.PROMPT_SCHEDULE_GEN
            )

            if config.INNER_VERSION in SpecifierSet(">=0.0.2"):
                config.PERSONALITY_1 = personality_config.get(
                    "personality_1_probability", config.PERSONALITY_1
                )
                config.PERSONALITY_2 = personality_config.get(
                    "personality_2_probability", config.PERSONALITY_2
                )
                config.PERSONALITY_3 = personality_config.get(
                    "personality_3_probability", config.PERSONALITY_3
                )

        def emoji(parent: dict):
            emoji_config = parent["emoji"]
            config.EMOJI_CHECK_INTERVAL = emoji_config.get(
                "check_interval", config.EMOJI_CHECK_INTERVAL
            )
            config.EMOJI_REGISTER_INTERVAL = emoji_config.get(
                "register_interval", config.EMOJI_REGISTER_INTERVAL
            )
            config.EMOJI_CHECK_PROMPT = emoji_config.get(
                "check_prompt", config.EMOJI_CHECK_PROMPT
            )
        ...
        def cq_code(parent: dict):
            ...
            config.ENABLE_PIC_TRANSLATE = cq_code_config.get(
                "enable_pic_translate", config.ENABLE_PIC_TRANSLATE
            )
        ...
        def response(parent: dict):
            ...
            config.MODEL_R1_PROBABILITY = response_config.get(
                "model_r1_probability", config.MODEL_R1_PROBABILITY
            )
            config.MODEL_V3_PROBABILITY = response_config.get(
                "model_v3_probability", config.MODEL_V3_PROBABILITY
            )
            config.MODEL_R1_DISTILL_PROBABILITY = response_config.get(
                "model_r1_distill_probability", config.MODEL_R1_DISTILL_PROBABILITY
            )
            config.max_response_length = response_config.get(
            ...
            for item in config_list:
                if item in model_config:
                    ...
                    if config.INNER_VERSION in SpecifierSet("<=0.0.0"):
                        ...
                        if provider == None:
                            logger.error(
                                f"provider 字段在模型配置 {item} 中不存在，请检查"
                            )
                            raise KeyError(
                                f"provider 字段在模型配置 {item} 中不存在，请检查"
                            )
        ...
        def message(parent: dict):
            ...
            config.MIN_TEXT_LENGTH = msg_config.get(
                "min_text_length", config.MIN_TEXT_LENGTH
            )
            config.MAX_CONTEXT_SIZE = msg_config.get(
            ...
            if config.INNER_VERSION in SpecifierSet(">=0.0.2"):
                config.thinking_timeout = msg_config.get(
                ...
                config.down_frequency_rate = msg_config.get(
                    "down_frequency_rate", config.down_frequency_rate
                )

        def memory(parent: dict):
            memory_config = parent["memory"]
            config.build_memory_interval = memory_config.get(
                "build_memory_interval", config.build_memory_interval
            )
            config.forget_memory_interval = memory_config.get(
                "forget_memory_interval", config.forget_memory_interval
            )

        def mood(parent: dict):
            mood_config = parent["mood"]
            config.mood_update_interval = mood_config.get(
                "mood_update_interval", config.mood_update_interval
            )
            config.mood_decay_rate = mood_config.get(
                "mood_decay_rate", config.mood_decay_rate
            )
            config.mood_intensity_factor = mood_config.get(
                "mood_intensity_factor", config.mood_intensity_factor
            )
        ...
        def keywords_reaction(parent: dict):
            ...
            if keywords_reaction_config.get("enable", False):
                config.keywords_reaction_rules = keywords_reaction_config.get(
                    "rules", config.keywords_reaction_rules
                )

        def chinese_typo(parent: dict):
            chinese_typo_config = parent["chinese_typo"]
            config.chinese_typo_enable = chinese_typo_config.get(
                "enable", config.chinese_typo_enable
            )
            config.chinese_typo_error_rate = chinese_typo_config.get(
                "error_rate", config.chinese_typo_error_rate
            )
            config.chinese_typo_min_freq = chinese_typo_config.get(
                "min_freq", config.chinese_typo_min_freq
            )
        ...
        def groups(parent: dict):
            ...
            config.talk_frequency_down_groups = set(
                groups_config.get("talk_frequency_down", [])
            )
            config.ban_user_id = set(groups_config.get("ban_user_id", []))

        def others(parent: dict):
            others_config = parent["others"]
            config.enable_advance_output = others_config.get(
                "enable_advance_output", config.enable_advance_output
            )
            config.enable_kuuki_read = others_config.get(
                "enable_kuuki_read", config.enable_kuuki_read
            )
        ...
        include_configs = {
            ...
            "memory": {"func": memory, "support": ">=0.0.0"},
        ...
        if os.path.exists(config_path):
            with open(config_path, "rb") as f:
                ...
                for key in include_configs:
                    if key in toml_dict:
                        group_specifierset: SpecifierSet = include_configs[key][
                            "support"
                        ]
                    ...
                    # 如果 necessary 项目存在，而且显式声明是 False，进入特殊处理
                        ...
                        and include_configs[key].get("necessary") == False
                    ):
