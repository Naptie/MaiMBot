...
class Database:
    ...
    @classmethod
    def initialize(
        cls, host, port, db_name, username=None, password=None, auth_source="admin"
    ):
        try:
...
def get_cloest_chat_from_db(db, length: int, timestamp: str):
    ...
    if closest_record and closest_record.get("memorized", 0) < 4:
        ...
        chat_records = list(
            db.db.messages.find({"time": {"$gt": closest_time}, "group_id": group_id})
            .sort("time", 1)
            .limit(length)
        )
...
class Hippocampus:
    ...
    async def memory_compress(self, input_text, compress_rate=0.1):
        ...
        topics_response = self.llm_model_get_topic.generate_response(
