...
class Memory_graph:
    ...
    def get_random_chat_from_db(self, length: int, timestamp: str):
        ...
        chat_text = ""
        closest_record = self.db.db.messages.find_one(
        ...
        if closest_record:
            ...
            for record in chat_record:
                time_str = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(int(record["time"]))
                )
                try:
                    displayname = "[(%s)%s]%s" % (
                        record["user_id"],
                        record["user_nickname"],
                        record["user_cardname"],
                    )
                except:
                    displayname = record["user_nickname"] or "用户" + str(
                        record["user_id"]
                    )
...
def visualize_graph(memory_graph: Memory_graph, color_by_memory: bool = False):
    ...
    title = "记忆图谱可视化 - " + (
        "按记忆数量着色" if color_by_memory else "按连接数量着色"
    )
