...
class Hippocampus:
    def __init__(self, memory_graph: Memory_graph):
        ...
        self.llm_topic_judge = LLM_request(
            model=global_config.llm_topic_judge, temperature=0.5
        )
        self.llm_summary_by_topic = LLM_request(
            model=global_config.llm_summary_by_topic, temperature=0.5
        )
    ...
    def get_memory_sample(
        self, chat_size=20, time_frequency: dict = {"near": 2, "mid": 4, "far": 3}
    ):
        ...
        for _ in range(time_frequency.get("near")):  # 循环10次
            ...
            chat_ = get_cloest_chat_from_db(
                db=self.memory_graph.db, length=chat_size, timestamp=random_time
            )
        ...
        for _ in range(time_frequency.get("far")):  # 循环10次
            random_time = current_timestamp - random.randint(
                3600 * 4, 3600 * 24
            )  # 随机时间
            chat_ = get_cloest_chat_from_db(
                db=self.memory_graph.db, length=chat_size, timestamp=random_time
    ...
    async def memory_compress(self, input_text, compress_rate=0.1):
        ...
        topics_response = await self.llm_topic_judge.generate_response(
            self.find_topic_llm(input_text, topic_num)
        )
        # 修改话题处理逻辑
        # 定义需要过滤的关键词
        ...
        topics = [
            topic.strip()
            for topic in topics_response[0]
            .replace("，", ",")
            .replace("、", ",")
            .replace(" ", ",")
            .split(",")
        ...
        filtered_topics = [
            topic
            for topic in topics
            if not any(keyword in topic for keyword in filter_keywords)
        ]
    ...
    def calculate_topic_num(self, text, compress_rate):
        ...
        topic_by_information_content = max(
            1, min(5, int((information_content - 3) * 2))
        )
        ...
        for i, input_text in enumerate(memory_sample, 1):
            ...
            for i in range(len(all_topics)):
                for j in range(i + 1, len(all_topics)):
                    print(
                        f"\033[1;32m连接节点\033[0m: {all_topics[i]} 和 {all_topics[j]}"
                    )
    ...
    def sync_memory_to_db(self):
            ...
            if concept not in db_nodes_dict:
                ...
                if db_hash != memory_hash:
                    self.memory_graph.db.db.graph_data.nodes.update_one(
                        {"concept": concept},
                        {"$set": {"memory_items": memory_items, "hash": memory_hash}},
                    )
        ...
        for db_node in db_nodes:
            if db_node["concept"] not in memory_concepts:
                self.memory_graph.db.db.graph_data.nodes.delete_one(
            ...
            if edge_key not in db_edge_dict:
                ...
                if db_edge_dict[edge_key]["hash"] != edge_hash:
                    self.memory_graph.db.db.graph_data.edges.update_one(
                        ...
                        {"$set": {"hash": edge_hash, "strength": strength}},
                    )
    ...
    def _find_similar_topics(
        self, topics: list, similarity_threshold: float = 0.4, debug_info: str = ""
    ) -> list:
        """查找与给定主题相似的记忆主题
    ...
    async def memory_activate_value(
        self, text: str, max_topics: int = 5, similarity_threshold: float = 0.3
    ) -> int:
        """计算输入文本对记忆的激活程度"""
        print(
            f"\033[1;32m[记忆激活]\033[0m 识别主题: {await self._identify_topics(text)}"
        )
        ...
        for memory_topic, similarity in top_topics:
            # 计算内容数量惩罚
            memory_items = self.memory_graph.G.nodes[memory_topic].get(
                "memory_items", []
            )
            if not isinstance(memory_items, list):
                memory_items = [memory_items] if memory_items else []
            ...
            for input_topic in identified_topics:
                ...
                if sim >= similarity_threshold:
                    ...
                    topic_similarities[input_topic] = max(
                        topic_similarities.get(input_topic, 0), adjusted_sim
                    )
                    print(
        ...
        average_similarities = (
            sum(topic_similarities.values()) / len(topic_similarities)
            ...
            if first_layer:
                ...
                for memory in first_layer:
                    relevant_memories.append(
                        {"topic": topic, "similarity": score, "content": memory}
                    )
