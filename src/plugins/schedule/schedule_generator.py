...
class ScheduleGenerator:
    def __init__(self):
        ...
        self.llm_scheduler = LLM_request(
            model=global_config.llm_normal, temperature=0.9
        )
    ...
    async def initialize(self):
        ...
        self.today_schedule_text, self.today_schedule = (
            await self.generate_daily_schedule(target_date=today)
        )
        self.tomorrow_schedule_text, self.tomorrow_schedule = (
            await self.generate_daily_schedule(target_date=tomorrow, read_only=True)
        )
        self.yesterday_schedule_text, self.yesterday_schedule = (
            await self.generate_daily_schedule(target_date=yesterday, read_only=True)
        )
        ...
        if existing_schedule:
            ...
            try:
                ...
                self.db.db.schedule.insert_one(
                    {"date": date_str, "schedule": schedule_text}
                )
    ...
    def print_schedule(self):
        ...
        if not self._parse_schedule(self.today_schedule_text):
            ...
            self.db.db.schedule.delete_one(
